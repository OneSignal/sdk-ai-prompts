...
## Non-Production API Host (Internal / Dogfooding Only)

> **⚠️ Internal / non-production only — must never ship in a production build.**
>
> By default the OneSignal SDK talks to **production** (`https://api.onesignal.com/`), which is correct for real apps — do nothing and you get production. Only add the override below when you are explicitly dogfooding against a non-production OneSignal environment (e.g. staging) using an App ID that does not exist on production.

There is **no public SDK API** for changing the API host. The technique below reaches into an **internal** SDK class (`ConfigModelStore`), so treat it as unsupported — it may change or break across SDK versions. Gate it behind a debug-only flag or build variant so it can never be enabled in a release build.

Two ordering rules are critical, and both must hold:

1. **Run it immediately *after* `initWithContext`, never before.** The config model store loads from `SharedPreferences` and needs the app context that `initWithContext` establishes; accessing it earlier throws a `NullPointerException`.
2. **Run it synchronously, on the same thread, right after init** — before the SDK's asynchronous params fetch issues its first request. That fetch (which carries the FCM sender ID) runs on a freshly spawned thread, so a synchronous post-init assignment on the init thread reliably wins the race. If it loses the race, the params request hits production, returns `403`, and FCM never configures.
...
// Pass apiHost ONLY for internal/non-production dogfooding
// (e.g. "https://api.staging.onesignal.com/"). Leave it null for production (the default).
// apiHost relies on an internal SDK class and must never be set in a release build.
fun initOneSignal(context: Context, appId: String, apiHost: String? = null) {
    OneSignal.initWithContext(context, appId)

    // Must run here: after init (the config store needs the app context) and
    // synchronously on this thread (to beat the async params fetch's first request).
    if (apiHost != null) {
        OneSignal.getService<ConfigModelStore>().model.apiUrl = apiHost
    }
}

// Production (default):  initOneSignal(context, "YOUR_ONESIGNAL_APP_ID")
// Staging (debug only):  initOneSignal(context, "YOUR_STAGING_APP_ID", "https://api.staging.onesignal.com/")
```
...
// Pass apiHost ONLY for internal/non-production dogfooding
// (e.g. "https://api.staging.onesignal.com/"). Pass null for production (the default).
// apiHost relies on an internal SDK class and must never be set in a release build.
public static void initOneSignal(Context context, String appId, String apiHost) {
    OneSignal.initWithContext(context, appId);

    // Must run here: after init (the config store needs the app context) and
    // synchronously on this thread (to beat the async params fetch's first request).
    if (apiHost != null) {
        OneSignal.INSTANCE.getServices()
            .getService(ConfigModelStore.class)
            .getModel()
            .setApiUrl(apiHost);
    }
}

// Production (default):  initOneSignal(context, "YOUR_ONESIGNAL_APP_ID", null);
// Staging (debug only):  initOneSignal(context, "YOUR_STAGING_APP_ID", "https://api.staging.onesignal.com/");
```
